...
def draw_dashed_line(surf, color, start_pos, end_pos, width=1, dash_length=10):
    ...
    # slope is a Point, so we can reuse operator overloads
...
def draw_dashed_line_delay(surf, color, start_pos, end_pos, width=1, dash_length=10, delay=0):
    ...
    # offset the origin by the delay
...
def draw_road_overlay_vision(surface, subject_car):
    ...
    # Draw on surface
    min_x = min(max(0, subject_car.lane - VISION_W - 1), 3)
    max_x = min(max(0, subject_car.lane + VISION_W - 1), 3)
...
def identify_free_lane(cars):
    lanes = [[n for n in range(1, 5)] for _ in range(2)]
    for car in cars:
        if -170 <= car.y <= 0:
...
# ============================================
# gui_util.py — drawing helpers for DeepTraffic
# ============================================
# Key changes:
# - No more hardcoded "7" lanes. Uses config.LANE_COUNT and LANE_INDEX_MAX.
# - Overlays (safety, vision) adapted for variable lane count.
# - Added comments to explain each block.
...
from config import (
    ...
    MISMATCH_ACTION_PENALTY,
    SWITCHING_LANE_PENALTY,
    LANE_COUNT,
    LANE_INDEX_MAX
)
...
def draw_dashed_line(surf, color, start_pos, end_pos, width=1, dash_length=10):
    ...
    slope = (displacement.x / length, displacement.y / length)
    loop = int(length / dash_length)

    for index in range(0, loop, 2):
        start = origin + Point(slope) * (index * dash_length)
        end = origin + Point(slope) * ((index + 1) * dash_length)
        gfxdraw.filled_polygon(surf, (
            (int(start.x), int(start.y)),
            (int(start.x) + width, int(start.y)),
            (int(end.x) + width, int(end.y)),
...
def draw_dashed_line_delay(surf, color, start_pos, end_pos, width=1, dash_length=10, delay=0):
    """Dashed line with initial delay (used for animations)."""
    if not VISUALENABLED:
        return
    ...
    origin = origin + Point(slope) * (delay * 10)
...
def draw_basic_road(surface, speed):
    """Draw the base road with side boundaries + dashed lane dividers."""
    if not VISUALENABLED:
        return
    ...
    pygame.draw.line(surface, black, (ROAD_VIEW_OFFSET + 13, -10), (ROAD_VIEW_OFFSET + 13, 1000), 5)
    # Right border
    right_edge = ROAD_VIEW_OFFSET + (LANE_COUNT * 50) + 15 + 2
    pygame.draw.line(surface, black, (right_edge, -10), (right_edge, 1000), 5)

    line_marking_offset = randint(0, 10)
    # Draw dashed lane dividers (LANE_COUNT - 1 gaps)
...
def draw_road_overlay_safety(surface, lane_map):
    """Highlight occupied cells in yellow (safety overlay)."""
    if not VISUALENABLED:
        return
...
def draw_road_overlay_vision(surface, subject_car):
    """Highlight subject car’s perception window in orange."""
    if not VISUALENABLED:
        return

    min_x = min(max(0, subject_car.lane - VISION_W - 1), LANE_INDEX_MAX)
    max_x = min(max(0, subject_car.lane + VISION_W - 1), LANE_INDEX_MAX)
...
def control_car(target_car, keydown):
    """Keyboard control: arrow keys map to accelerate/brake/steer."""
    if keydown == pygame.K_UP:
        target_car.move('A')
    elif keydown == pygame.K_DOWN:
        target_car.move('D')
...
def identify_free_lane(cars):
    """Return free lanes (front and back segments) for spawning new cars."""
    lanes = [[n for n in range(1, LANE_COUNT + 1)] for _ in range(2)]
    for car in cars:
        if -170 <= car.y <= 0:
            if car.lane in lanes[0]:
                lanes[0].remove(car.lane)
...
def draw_inputs(surface, vision):
    """Draw CNN input matrix (orange squares = occupied cells)."""
    vision_title = font_28.render("Vision:", False, (0, 0, 0))
    surface.blit(vision_title, (INPUT_VIEW_OFFSET_X - 10, INPUT_VIEW_OFFSET_Y))
    for y_i in range(len(vision)):
        for x_i, x in enumerate(vision[y_i]):
...
def draw_actions(surface, action):
    """Draw last action executed (icons)."""
    action_title = font_28.render("Action:", False, (0, 0, 0))
    surface.blit(action_title, (INPUT_VIEW_OFFSET_X - 10, INPUT_VIEW_OFFSET_Y + 370))
...
class Score:
    """Encapsulates scoring logic (reward shaping)."""
    def __init__(self, score=0):
        self.score = score
    ...
    def penalty(self):
        self.score += CONSTANT_PENALTY

    def brake_penalty(self):
        self.score += EMERGENCY_BRAKE_PENALTY

    def action_mismatch_penalty(self):
        self.score += MISMATCH_ACTION_PENALTY
...
def draw_gauge(surface, speed):
    """Draw speed gauge using PIL gauge renderer."""
    im = Image.new("RGB", (200, 200), (255, 255, 255, 0))
    from advanced_view.gauge import GaugeDraw
    g = GaugeDraw(im, 0, 110)
    g.render_simple_gauge(value=speed, major_ticks=10, minor_ticks=5, label="{}kmh".format(speed))
